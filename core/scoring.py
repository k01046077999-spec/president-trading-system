...
def score_candidate(
    *,
    ...
    if volume_ratio_value > 0.08:
        ...
        if mode == "main":
            downgrade_reasons.append("메인 기준 거래량 보강 부족")
    ...
    if structure_score >= 2:
        ...
        if mode == "main":
            downgrade_reasons.append("메인 기준 구조 점수 부족")
    ...
    rr_cut = 1.6 if mode == "main" else 1.1
    if rr1 >= rr_cut:
        score += 2
    ...
    passed = score >= (8 if mode == "main" else 4) and not hard_fail_reasons
    return {
        "score": round(score, 2),
        "passed": passed,
        "reasons": reasons,
